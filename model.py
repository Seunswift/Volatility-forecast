...
class GarchModel:
    ...
    def wrangle_data(self, n_observations):
        ...
        df = self.repo.read_table(table_name=self.ticker, limit = n_observations + 1) # we add 1 to replace the first Nan from calculating the return
    ...
    def fit(self, p, q):
        ...
        self.aic = round(self.model.aic, 2)
        self.bic= round(self.model.bic, 2)
        

    def __clean_prediction(self, prediction): # helper function __ user can't access:
        ...
        # Create date range
        prediction_dates = pd.bdate_range(start=start, periods =prediction.shape[1]) #bdate_range is business day (no weekends), periods refer to the no of date

        # Create prediction index labels, ISO 8601 format
        prediction_index = [d.isoformat() for d in prediction_dates]

        # Extract predictions from DataFrame, get square root
        data = prediction.values.flatten() ** 0.5 # flattens reduce the array from double to single
    ...
    def dump(self):
        ...
        filepath = os.path.join(self.model_directory, f"{timestamp}_{self.ticker}.pkl")  #os.path puts the /
    ...
    """
        ...
        # Handle possible `IndexError`
            ...
            raise Exception(f"No model has this '{ticker}'.")
